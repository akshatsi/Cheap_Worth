...
- The routed tiers (`app/models/execute.py`) call a local Ollama server, not a paid API — no key, no metered cost, cost/savings numbers for them read $0 by design. The classifier (`app/models/classify.py`) is still a real Groq call. When touching orchestration or model-client code, notice if a change adds a call that wasn't there before, and say so — this project's whole point is minimizing that.
